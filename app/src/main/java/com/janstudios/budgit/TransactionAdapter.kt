...
class TransactionAdapter(private val transactions: ArrayList<Transaction>) : RecyclerView.Adapter<TransactionAdapter.TransactionHolder>(){
    ...
    override fun onBindViewHolder(holder: TransactionHolder, position: Int) {
        ...
        if (transaction.amount >= 0){
            ...
            holder.amount.setTextColor(ContextCompat.getColor(context, com.google.android.material.R.color.design_dark_default_color_primary))
        }

        else{
            holder.amount.text = "- $%.2f".format(Math.abs(transaction.amount))
            holder.amount.setTextColor(ContextCompat.getColor(context, com.google.android.material.R.color.design_default_color_error))
        }
